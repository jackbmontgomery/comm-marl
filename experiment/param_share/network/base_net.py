    ...
    def forward(self, obs, hidden_state, msgs=None, agent_num=None):
        ...
        if self.args.with_comm:
            ...
            if agent_num != None:
                ...
                # TO RECEIVE ALL MSGS
                idxs = torch.tensor(
                    [i for i in range(self.args.n_agents) if i != agent_num]
                ).to("cpu")
                ...
                a_mask = torch.eye(self.args.n_agents).reshape(
                    self.args.n_agents, self.args.n_agents, 1
                )
                a_mask = torch.abs(a_mask - 1)

                # msg_rec: [bs, n_agents, msg_dim]
                msgs_rec_rep = msgs_rec.repeat(1, self.args.n_agents, 1).reshape(
                    ep_num, self.args.n_agents, self.args.n_agents, -1
                )
                ...
                msgs_repective_idxs_no_0 = msgs_repective_idxs[
                    msgs_repective_idxs.count_nonzero(dim=-1) != 0
                ].reshape(ep_num, self.args.n_agents, -1)
                ...
                obs = torch.cat((obs_aux, msgs_rec), dim=-1).reshape(
                    ep_num * self.args.n_agents, -1
                )
