...
class Agents:
    ...
    def choose_action(
        ...
        if self.args.last_action:
            inputs = np.hstack(
                (inputs, last_action)
            )  # concatenates arrays column wise (horizontally)
        ...
        if self.args.with_comm:
            ...
            q_value, self.policy.eval_hidden[:, agent_num, :] = self.policy.eval_rnn(
                inputs, hidden_state, msg_all, agent_num
            )
        else:
            q_value, self.policy.eval_hidden[:, agent_num, :] = self.policy.eval_rnn(
                inputs, hidden_state
        ...
        if self.args.alg == "coma":
            ...
            if np.random.uniform() < epsilon:
                action = np.random.choice(
                    avail_actions_ind
                )  # picks an action from the available actions array
            else:
    ...
    def train(self, batch, train_step, epsilon=None):
        ...
        max_episode_len = self._get_max_episode_len(
            batch
        )  # inside batch there are several episode batches; as they may have different sizes, gets the bigger
    ...
    def _choose_action_from_softmax(
        self, inputs, avail_actions, epsilon, evaluate=False
    ):
    ...
    def _get_max_episode_len(self, batch):
        ...
        for episode_idx in range(episode_num):
            for transition_idx in range(self.args.episode_limit):
                if (
                    terminated[episode_idx, transition_idx, 0] == 1
                ):  # TODO: better understand this 3D shape; prob is [buffer_size, episode_limit, ?]; transition idx refers to the trajectory, i.e., passage from one state to another inside this episode
                    if transition_idx + 1 >= max_episode_len:
                        max_episode_len = transition_idx + 1
                    break
        return max_episode_len
