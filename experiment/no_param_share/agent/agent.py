...
class Agents:
    ...
    def choose_action(
        ...
        if self.args.with_comm:
            ...
            q_value, self.policy.eval_hidden = self.policy.eval_rnn(
                inputs, hidden_state, msg_all, agent_num, msg_i
            )
        else:
            q_value, self.policy.eval_hidden = self.policy.eval_rnn(
                inputs, hidden_state
    ...
    def get_msgs_for_train(self, batch, train_step, agent_id, epsilon=None):
        ...
        msgs_agent_i, msgs_agent_i_next = self.policy.generate_msgs_agent_i(
            batch, max_episode_len, agent_id
        )
