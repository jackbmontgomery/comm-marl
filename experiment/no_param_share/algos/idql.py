...
class IDQL:
    def __init__(self, args):
        ...
        if args.with_comm:
            input_shape += (
                args.n_agents
            ) * args.final_msg_dim  # (args.n_agents - 1) * args.final_msg_dim
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + "/rnn_net_params.pkl"):
                ...
                print(
                    "Successfully load the model: {} and {}".format(path_rnn, path_vdn)
                )
            else:
    ...
    def learn(
        ...
        (
            ...
            batch["obs"][:, :, agent_id],
            batch["obs_next"][:, :, agent_id],
            batch["actions"][:, :, agent_id],
            batch["reward"],
            batch["avail_actions"][:, :, agent_id],
            batch["avail_actions_next"][:, :, agent_id],
            batch["terminated"],
        )
        ...
        q_evals, q_targets = self.get_q_values(
            batch, max_episode_len, agent_id, all_msgs, all_msgs_next
        )
    ...
    def generate_msgs_agent_i(self, batch, max_episode_len, agent_id):
        ...
        for transition_idx in range(max_episode_len):
            all_msgs, all_msgs_next = self.get_msgs(
                batch, transition_idx, agent_id
            )  # add last action and agent_id to the obs
    ...
    def get_msgs(self, batch, transition_idx, agent_id):
        obs, obs_next, actions_onehot = (
            batch["obs"][:, transition_idx, agent_id],
            batch["obs_next"][:, transition_idx, agent_id],
    ...
    def get_q_values(self, batch, max_episode_len, agent_id, all_msgs, all_msgs_next):
        ...
        if self.args.with_comm:
            ...
            others_msgs_next = [
                all_msgs_next[i] for i in range(self.n_agents) if i != agent_id
            ]
        ...
        for transition_idx in range(max_episode_len):
            ...
            if self.args.with_comm:
                ...
                others_msgs_tidx = others_msgs[
                    :, :, transition_idx, :
                ]  # select time step, all_msgs contains from all of them  [n_ep, n_a, ts, dim]
                others_msgs_next_tidx = others_msgs_next[
                    :, :, transition_idx, :
                ]  # select time step, all_msgs contains from all of them  [n_ep, n_a, ts, dim]
            ...
            if self.args.with_comm:
                ...
                q_eval, self.eval_hidden = self.eval_rnn(
                    inputs,
                    self.eval_hidden,
                    msgs=others_msgs_tidx,
                    agent_num=agent_id,
                    msg_i=msg_agent_tidx,
                )
                q_target, self.target_hidden = self.target_rnn(
                    inputs_next,
                    self.target_hidden,
                    msgs=others_msgs_next_tidx,
                    agent_num=agent_id,
                    msg_i=msg_agent_next_tidx,
                )
            else:
    ...
    def _get_inputs(self, batch, transition_idx, agent_id):
        ...
        if self.args.last_action:
            if (
                transition_idx == 0
            ):  # if it is the first transition, let the previous action be a 0 vector
                inputs.append(
                    torch.zeros_like(actions_onehot[:, transition_idx, agent_id])
                )
            else:
        ...
        inputs_next = torch.cat(
            [x.reshape(episode_num, -1) for x in inputs_next], dim=1
        )
    ...
    def save_model(self, train_step, agent_id, end_training=False):
        ...
        if end_training:
            torch.save(
                ...
                self.model_dir + "/" + f"final_rnn_net_params_{agent_id}.pkl",
            )
        else:
            ...
            torch.save(
                self.eval_rnn.state_dict(),
                self.model_dir + "/" + num + f"_rnn_net_params_{agent_id}.pkl",
            )
